...
static uint64_t g_auxv_base = 0;       // AT_BASE: ld.so 第一个 PT_LOAD 段的实际加载地址
...
// 已通过 dlopen/加载逻辑映射进内存的共享库描述（链表节点）
typedef struct loaded_library {
    char *path;                    /* 库路径字符串（如 "/lib/libc.so.6"） */
    uint64_t base_addr;            /* 第一个 PT_LOAD 的实际加载基址（类似 AT_BASE） */
    Elf64_Ehdr *ehdr;              /* 映射后 ELF 头指针（通常在 base_addr） */
    Elf64_Phdr *phdr;              /* 程序头表指针（用于 vaddr→内存地址换算） */
    Elf64_Dyn *dynamic;            /* .dynamic 数组指针（由 PT_DYNAMIC 定位） */
    const char *strtab;            /* 动态字符串表 .dynstr 的实际内存地址 */
    Elf64_Sym *symtab;             /* 动态符号表 .dynsym 的实际内存地址 */
    uint32_t symnum;               /* 符号表项数量（通常由 DT_SYMTAB 与 DT_HASH 推算） */
    uint64_t init;                 /* DT_INIT：库初始化函数虚拟地址，0 表示无 */
    uint64_t init_array;           /* DT_INIT_ARRAY：初始化函数指针数组地址 */
    uint64_t init_array_size;      /* DT_INIT_ARRAYSZ：init_array 字节长度 */
    struct loaded_library *next;     /* 下一已加载库，构成全局链表 g_loaded_libraries */
} LoadedLibrary;
                                         ...
                                         Elf64_Phdr *phdr, uint16_t phnum)
{
    ...
    // 按 vaddr 换算：memaddr = base_addr + (st_value - first_load_vaddr)
    uint64_t sym_addr = 0;
    ...
    for (uint16_t i = 0; i < phnum; i++) {
        ...
        if (load->p_type == PT_LOAD) {
            if (sym_entry->st_value >= load->p_vaddr &&
                ...
                sym_addr = base_addr + (sym_entry->st_value - first_load_vaddr);
                break;
            }
        }
    }
...
/**
 * 通过 LOAD 段映射关系，将链接时虚拟地址换算为实际内存地址
 *
 * @param vaddr     链接时虚拟地址
 * @param base_addr 第一个 PT_LOAD 段的实际加载地址（自举时为 AT_BASE）
 * @param phdr      程序头表
 * @param phnum     程序头表项数量
 * @return 实际内存地址；找不到包含该 vaddr 的 LOAD 段时返回 0
 */
                                             ...
                                             Elf64_Phdr *phdr, uint16_t phnum)
{
    ...
    for (uint16_t i = 0; i < phnum; i++)
    {
        ...
        if (load->p_type == PT_LOAD)
        {
            ...
            {
                // 通用地址计算：内核与本链接器均按 PT_LOAD 分段映射，非整文件拷贝
                // base_addr 均为第一个 LOAD 段的实际加载地址：
                //   - 自举：g_auxv_base（AT_BASE，ld.so 首段加载地址）
                //   - 其他库：load_shared_library 映射后的 base_addr
                //
...
/**
 * 处理单条 RELA 重定位：解析 r_offset 得到待修补位置，按类型写入最终值。
 *
 * @param rela      当前重定位条目（.rela.dyn 或 .rela.plt 中的一项）
 * @param base_addr 本 ELF 第一个 PT_LOAD 的实际加载地址（自举时为 g_auxv_base/AT_BASE）
 * @param symtab    动态符号表 .dynsym，按 r_sym 查 Elf64_Sym（ABS64/GLOB_DAT/JUMP_SLOT 需要）
 * @param strtab    动态字符串表 .dynstr，由 st_name 取符号名（JUMP_SLOT 跨库解析需要）
 * @param symnum    symtab 有效项数上界，校验 r_sym 索引
 * @param got       .got.plt 运行时地址（DT_PLTGOT）；预留供 PLT/GOT 相关重定位，当前未使用
 * @param ehdr      本 ELF 头；与 phdr 配合将 r_offset/st_value 等 vaddr 换算为内存指针
 * @param phdr      程序头表；用于 calculate_address_from_vaddr / calculate_symbol_address
 */
                                ...
                                Elf64_Ehdr *ehdr, Elf64_Phdr *phdr)
{
    ...
    uint64_t sym = ELF64_R_SYM(rela->r_info); // 获取符号索引
    uint64_t offset = rela->r_offset;  // r_offset是待修补的虚拟地址（vaddr）
    
    // 计算目标地址：按 PT_LOAD 的 vaddr 映射换算（可执行文件/ld.so/共享库均如此）
    // base_addr 为第一个 LOAD 段的加载地址；实际地址 = base_addr + (vaddr - first_load_vaddr)
    uint64_t *addr = NULL;
...
static int bootstrap_self(void)
{
    ...
    // 程序头表地址计算：
    //
    // 内核加载 ld.so 与可执行文件、共享库一样，只 mmap 各 PT_LOAD 段，非整文件映射。
    // g_auxv_base（AT_BASE）是第一个 LOAD 段的实际加载地址。
    //
    // phdr 通常落在第一个 LOAD 内（且常见 p_vaddr=0、p_offset=0），故：
    //   phdr = g_auxv_base + e_phoff
    // 在 x86-64 常见布局下成立；更通用的换算是按 LOAD 段的 p_vaddr/p_offset 关系。
    //
    // 自举与 load_shared_library 的区别不在“是否连续加载”，而在于：
    //   - 自举：内核已映射 ld.so，直接复用 AT_BASE
    //   - 其他库：本链接器自行逐段 mmap（见 load_shared_library）
    ...
    for (uint16_t i = 0; i < ehdr->e_phnum; i++)
    {
        ...
        if (p->p_type == PT_DYNAMIC)
        {
            ...
            else
            {
                ...
                // dynamic段的虚拟地址范围是：[p_vaddr, p_vaddr + p_memsz)
    ...
    // 从 .dynamic 读取自举重定位所需的关键条目（返回值均为 d_val；地址类为虚拟地址，其余为字节数/大小）
    // DT_STRTAB：.dynstr 字符串表虚拟地址，符号名、依赖库名等通过 st_name 偏移在此查找
    uint64_t strtab_addr = get_dynamic_tag((Elf64_Dyn *)dynamic, DT_STRTAB);
    // DT_SYMTAB：.dynsym 动态符号表虚拟地址，重定位时按 r_sym 索引 Elf64_Sym 并解析目标
    uint64_t symtab_addr = get_dynamic_tag((Elf64_Dyn *)dynamic, DT_SYMTAB);
    // DT_RELA：.rela.dyn 重定位表虚拟地址，含 GLOB_DAT、RELATIVE 等非 PLT 条目
    uint64_t rela_addr = get_dynamic_tag(dynamic, DT_RELA);
    // DT_RELASZ：.rela.dyn 总字节数，除以 rela_ent 得条目个数
    uint64_t rela_size = get_dynamic_tag(dynamic, DT_RELASZ);
    // DT_RELAENT：单条 Elf64_Rela 的字节大小（x86-64 通常为 24），rela 与 pltrel 共用
    uint64_t rela_ent = get_dynamic_tag(dynamic, DT_RELAENT);
    // DT_JMPREL：.rela.plt 虚拟地址，PLT/GOT 延迟绑定相关（如 R_X86_64_JUMP_SLOT）
    uint64_t pltrel_addr = get_dynamic_tag(dynamic, DT_JMPREL);
    // DT_PLTRELSZ：.rela.plt 总字节数
    uint64_t pltrel_size = get_dynamic_tag(dynamic, DT_PLTRELSZ);
    // DT_PLTGOT：.got.plt 虚拟地址，过程链接表 GOT，PLT 跳转与重定位的最终写入目标
    ...
    // 将 strtab/symtab 的链接时虚拟地址换算为实际内存指针。
    //   memaddr = g_auxv_base + (vaddr - first_load_vaddr)
    // g_auxv_base（AT_BASE）锚定第一个 LOAD 在内存中的位置；各 LOAD 按 p_vaddr 相对首段映射。
    uint64_t strtab_memaddr = g_auxv_base + (strtab_addr - first_load_vaddr);
    uint64_t symtab_memaddr = g_auxv_base + (symtab_addr - first_load_vaddr);
...
static LoadedLibrary *load_shared_library(const char *libname)
{
    ...
    // 按 PT_LOAD 逐段 mmap（与内核加载可执行文件/ld.so 的方式一致，非整文件映射）
    for (uint16_t i = 0; i < ehdr.e_phnum; i++)
    {
        if (phdr[i].p_type == PT_LOAD)
        {
...
__attribute__((visibility("hidden"))) // 避免 main 函数进入动态符号表，被外部链接而产生错误
